...
public class Game {
    ...
    Game() {
        ...
        this.loadObjects();
        Thread thr1 = new Thread(turn());
        thr1.start();
        this.loop();
//        @SuppressWarnings("LeakingThisInConstructor")
//        Thread thr2 = new Thread(refreshLoop(this));
//
//        thr2.start();
    }

    public static Runnable turn() {
        Runnable r1 = new Runnable() {

            public void run() {
                while (!Helper.gameOver()) {
                    Player currentPlayer = Data.player;
//                currentPlayer = Data.players.get(Data.counter++ % Data.players.size() + 1);
                    if (!currentPlayer.isIa()) {
                        while (Data.player == currentPlayer) {
                        }
                    }
                    currentPlayer = Data.player;

                    if (currentPlayer.getPossibleMoves().isEmpty() && currentPlayer.getArmyAvaiable() == 0) {
                        currentPlayer.passTurn();
                    } else {
                        currentPlayer.receiveRoundArmy();
    ...
    public static Runnable refreshLoop(final Game game) {
        Runnable r1 = new Runnable() {
    ...
    private void startGame() {
        ...
        for (Player current_player : Data.players.values()) {
            current_player.receiveRoundArmy();
            current_player.initDistribution();
        }
        Player current_player = Data.players.get((++Data.counter % Data.players.size()) + 1);
...
//        while (!Helper.gameOver()) {
//            current_player = Data.players.get(nextPlayer++ % Data.players.size() + 1);
//            Data.player = current_player;
//            if (!current_player.getPossibleMoves().isEmpty()) {
//                current_player.receiveRoundArmy();
//                Data.status = Status.DISTRIBUTING;
